...
const char DOWNLOAD_URL[]="http://sourceforge.net/projects/javawords/files/latest/download";
//const char DOWNLOAD_URL[]="https://sourceforge.net/projects/javawords/files/";//all versions

const char VERSION_FILE_URL[] = "http://slovesnov.users.sf.net/words/version.txt";
//const char VERSION_FILE_URL[] = "D://version.txt";
...
static void entry_insert (GtkWidget* entry, gchar* new_text, gint new_text_length, gpointer position, int entryIndex){
	frame->entryChanged(entryIndex);
}

static void entry_delete (GtkWidget* entry, gint start_pos, gint end_pos, int entryIndex){
	frame->entryChanged(entryIndex);
}
...
Frame::Frame() : WordsBase("words/") {
  ...
  g_mutex_init(&m_mutex);
	...
	gtk_text_buffer_create_tag(gtk_text_view_get_buffer(GTK_TEXT_VIEW(m_text)), markTag, "background", "lightblue", NULL);
	gtk_text_buffer_create_tag(gtk_text_view_get_buffer(GTK_TEXT_VIEW(m_text)), activeTag, "background", "Khaki", NULL);
...
*/
	...
	createTextCombo(COMBOBOX_SORT,0,1,1);//Note 2nd parameter=0 because m_comboLowValue[COMBOBOX_SORT] should be = 0
	...
	for(i=0;i<int(G_N_ELEMENTS(m_searchButton));i++){
		...
		gtk_button_set_image(GTK_BUTTON(m_searchButton[i]),gtk_image_new_from_file(getImagePath(i==0 ? "down.png" : "up.png" ).c_str()) );
	}
  ...
  w = gtk_box_new (GTK_ORIENTATION_VERTICAL, 0);
	...
	while(fgets(buff,MAX_BUFF_LEN,f)!=NULL){
		...
		if(startsWith(buff,SEPARATOR)){
			gtk_menu_shell_append( GTK_MENU_SHELL(gtk_menu_item_get_submenu(subMenu.back())), gtk_separator_menu_item_new() );
			continue;
		}
		...
		if( (j=INDEX_OF(ICON_MENU,ENUM_MENU(i)))!=-1 ){
			...
			gtk_container_add (GTK_CONTAINER (w), gtk_image_new_from_file(getImagePath(ICON_MENU_FILE_NAME[j]).c_str()) );
		...
		gtk_menu_shell_append(GTK_MENU_SHELL( subMenu.size()==0 && bSubMenu ? m_menu : gtk_menu_item_get_submenu(subMenu.back()) ), item);

		if( (j=INDEX_OF(MENU_ACCEL,ENUM_MENU(i)))!=-1 ){
			gtk_widget_add_accelerator(item, "activate", m_accelGroup[j],ACCEL_KEY[j], GDK_CONTROL_MASK, GTK_ACCEL_VISIBLE);
		}
  ...
  GdkScreen *screen;
	...
	gtk_css_provider_load_from_path (provider,g_filename_to_utf8(home, strlen(home), NULL, NULL, NULL),NULL);
  ...
  connectEntrySignals(SEARCH_ENTRY_ID);
	...
	for(i=0;i<int(G_N_ELEMENTS(m_searchButton));i++){
		g_signal_connect(m_searchButton[i], "clicked",G_CALLBACK(button_clicked), NULL );
	}

	g_signal_connect(m_combo[COMBOBOX_SORT_ORDER], "changed",G_CALLBACK(combo_changed), gpointer(COMBOBOX_SORT_ORDER) );
	//after setComboIndex() & setDictionary()
	g_signal_connect(m_combo[COMBOBOX_DICTIONARY], "changed",G_CALLBACK(combo_changed), gpointer(COMBOBOX_DICTIONARY) );
	...
	m_newVersion.init(GTK_WINDOW(m_widget),m_language[PROGRAM_VERSION],VERSION_FILE_URL,DOWNLOAD_URL,WORDS_VERSION,m_language[NEW_VERSION_MESSAGE]);
}
...
void Frame::clickMenu(ENUM_MENU menu) {
  ...
  char*text;
	...
	case MENU_LOAD_RUSSIAN_DICTIONARY:
		setComboIndex( COMBOBOX_DICTIONARY, menu==MENU_LOAD_ENGLISH_DICTIONARY ? 0:1 );
	break;
...
void Frame::destroy(){
	...
	assert(f!=NULL);
	fprintf(f,"%s %s",getShortLanguageString(getDictionaryIndex()).c_str(),getShortLanguageString(m_languageIndex).c_str() );
	fclose(f);
...
void Frame::aboutDialog() {
  ...
  hbox = gtk_box_new (GTK_ORIENTATION_HORIZONTAL, 5 );
	gtk_container_add (GTK_CONTAINER (hbox), gtk_image_new_from_file(getImagePath("word111.png").c_str()));
  ...
  ENUM_STRING id;
	for(i=0 ; i<G_N_ELEMENTS(sid) ; i++ ){
		...
		if(id==STRING_SIZE){
			...
			s=format("build %.*s %s %s %s, gcc version %d.%d.%d, gtk version %d.%d.%d "
		...
		if(id==HOMEPAGE_STRING || id==HOMEPAGE_ONLINE_STRING){
			...
			markup = g_markup_printf_escaped ("%s <a href=\"%s,%s\">\%s,%s</a>"
					,m_language[id].c_str() ,p,LANGUAGE[m_languageIndex].c_str(), p,LANGUAGE[m_languageIndex].c_str() );
			gtk_label_set_markup (GTK_LABEL (label), markup);
			g_free (markup);
			g_signal_connect(label, "activate-link",G_CALLBACK(label_clicked), gpointer(p) );
...
void Frame::setHelperPanel() {
	...
	case MENU_MODIFICATION:
		m_check=gtk_check_button_new_with_label(m_language[EVERY_MODIFICATION_CHANGES_WORD].c_str());
	...
	case MENU_DOUBLE_WORD_SEQUENCE:
		addComboLineToHelper(LENGTH, 2, MAX_DOUBLE_WORD_SEQUENCE_LENGTH, 2, CHARACTERS);
		break;
	...
	case MENU_CONSONANT_VOWEL_SEQUENCE:
		...
		add(w,createTextCombo(COMBOBOX_HELPER0,SEARCH_IN_ANY_PLACE_OF_WORD,SEARCH_IN_END_OF_WORD,0));
...
void Frame::loadAndUpdateCurrentLanguage() {
	...
	gtk_widget_set_sensitive(m_menuMap[MENU_ENGLISH_LANGUAGE],m_languageIndex!=0);
	gtk_widget_set_sensitive(m_menuMap[MENU_RUSSIAN_LANGUAGE],m_languageIndex!=1);

	gtk_label_set_text(GTK_LABEL(m_searchLabel),m_language[SEARCH].c_str());
	gtk_label_set_text(GTK_LABEL(m_currentDictionary),m_language[DICTIONARY].c_str());
  gtk_window_set_title(GTK_WINDOW(m_widget),m_language[PROGRAM_VERSION].c_str());

  j=getComboIndex(COMBOBOX_SORT);
  lockSignals();
  gtk_combo_box_text_remove_all ( GTK_COMBO_BOX_TEXT(m_combo[COMBOBOX_SORT]) );
	for(i=0;i<NUMBER_OF_SORTS;i++){
		gtk_combo_box_text_append_text  ( GTK_COMBO_BOX_TEXT(m_combo[COMBOBOX_SORT]), m_language[SORT_BY_ALPHABET+i].c_str() );
	}
  setComboIndex(COMBOBOX_SORT,j);
  unlockSignals();
}

GtkWidget* Frame::createTextCombo(ENUM_COMBOBOX e, StringVector v, int active) {
	assert(e!=COMBOBOX_SIZE);

	GtkWidget*w=m_combo[e]=gtk_combo_box_text_new();
	StringVectorCI it;
	for(it=v.begin();it!=v.end();it++){
		gtk_combo_box_text_append_text(GTK_COMBO_BOX_TEXT(w), it->c_str() );
	}
...
GtkWidget* Frame::createTextCombo(ENUM_COMBOBOX e, int from, int to, int active) {
	StringVector v;
  int i;
	for(i=from;i<=to;i++){
		v.push_back(format("%d",i));
	}
	return createTextCombo(e,v,active);
}

GtkWidget* Frame::createTextCombo(ENUM_COMBOBOX e, ENUM_STRING from, ENUM_STRING to,int active) {
	StringVector v;
  int i;
	for(i=from;i<=to;i++){
...
void Frame::addComboLineToHelper(ENUM_STRING id, int from, int to, int active, ENUM_STRING eid) {
	addComboLineToHelper(id,from,to,active,m_language[id]+" "+m_language[FROM],m_language[TO],eid==STRING_SIZE?"":m_language[eid]);
}

void Frame::addComboLineToHelper(ENUM_STRING id, int from, int to, int active, std::string s1, std::string s2, std::string s3){
	int i;
...
void Frame::addComboToHelper(ENUM_STRING from, ENUM_STRING to, int active) {
	gtk_container_add (GTK_CONTAINER (m_helperUp),createTextCombo(COMBOBOX_HELPER0,from,to,active));
}
...
void Frame::comboChanged(ENUM_COMBOBOX e) {
	...
	if(e==COMBOBOX_SORT || e==COMBOBOX_SORT_ORDER || e==COMBOBOX_DICTIONARY){
		waitThread();
		...
		stopThread();
		if( (e==COMBOBOX_HELPER0 || e==COMBOBOX_HELPER1) && IN_ARRAY(MENU_ADJUST_COMBO,m_menuClick) ){
			if( getComboIndex(COMBOBOX_HELPER0) > getComboIndex(COMBOBOX_HELPER1) ){
				lockSignals();
				setComboIndex( e==COMBOBOX_HELPER0 ? COMBOBOX_HELPER1 : COMBOBOX_HELPER0, getComboIndex(e));
				unlockSignals();
			}
		}
...
void Frame::updateTags() {
	...
	for (	m_tags=0 ; gtk_text_iter_forward_search(&first, text
			,GtkTextSearchFlags(GTK_TEXT_SEARCH_TEXT_ONLY | GTK_TEXT_SEARCH_VISIBLE_ONLY)
			,&start, &end, NULL) ; m_tags++) {

		if(m_tags==m_tagIndex){
			scroll=start;
		}
		gtk_text_buffer_apply_tag_by_name(buffer, m_tags==m_tagIndex ? activeTag:markTag,&start, &end);
		gint offset = gtk_text_iter_get_offset(&end);
		gtk_text_buffer_get_iter_at_offset(buffer,&first, offset);
	}

	if(m_tags>0){
		gtk_text_view_scroll_to_iter (GTK_TEXT_VIEW(m_text),&scroll,0.0,true,.5,.5);
	}
	gtk_label_set_text(GTK_LABEL(m_searchTagLabel),format("%d/%d",m_tags==0 ? 0 : m_tagIndex+1,m_tags).c_str());
...
void Frame::createImageCombo(ENUM_COMBOBOX e) {
	...
	for(i=0;i<2;i++){
		...
		gtk_list_store_set( gls, &iter, 0,
				gdk_pixbuf_new_from_file(	getImagePath(image[j+i]).c_str(),NULL), -1 );
	}
	...
	gtk_cell_layout_set_attributes( GTK_CELL_LAYOUT(m_combo[e]), renderer, "pixbuf", 0, NULL );
}
...
void Frame::clickButton(GtkWidget* button){
	...
	assert(i!=int(G_N_ELEMENTS(m_searchButton)));
...
void Frame::updateComboValue(ENUM_COMBOBOX e) {
	...
	if( GTK_IS_COMBO_BOX_TEXT(m_combo[e]) ){
		char* p=gtk_combo_box_text_get_active_text (GTK_COMBO_BOX_TEXT(m_combo[e]));
		char* pEnd;
		long l = strtol (p,&pEnd,10);
		if(*pEnd==0){//full string recognized
			assert(l>=0);
			v=l;
		}
	}
	if(v==-1){
		v=getComboIndex(ENUM_COMBOBOX(e));
		assert( v!=-1 );
	}
	m_comboValue[e]=v;
}
...
bool Frame::prepare() {
	...
	if(m_menuClick==MENU_MODIFICATION){
		m_checkValue=gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON(m_check))==TRUE;
	}
...
void Frame::connectEntrySignals(int index) {
	GtkWidget*w = index==SEARCH_ENTRY_ID ? m_searchEntry : m_entry;
	g_signal_connect_after( G_OBJECT(w), "insert-text", G_CALLBACK(entry_insert), GP(index) );
	g_signal_connect_after( G_OBJECT(w), "delete-text", G_CALLBACK(entry_delete), GP(index) );
	g_signal_connect_after( G_OBJECT(w), "focus-in-event", G_CALLBACK(entry_focus_in), GP(index) );
	g_signal_connect_after( G_OBJECT(w), "focus-out-event", G_CALLBACK(entry_focus_out), GP(index) );
}
